...
cell_clustering <- function(count_data, percent=0.1, ncores=4, perplexity = 30,
  ...
  data_out <- Rtsne(data, 2, perplexity = perplexity)

  cl_out<- kmeans(data_out$Y[,1:2], kcluster)

  aRI <- adjustedRandIndex(labels, cl_out$cluster)
  ...
  cols <- c("#e78ac3", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7",
            "#8dd3c7", "#ffd92f", "#e78ac3", "#bebada", "#80b1d3", "#fc8d62", "#b3de69")

  plot(data_out$Y[,1:2], col = alpha(cols[as.numeric(as.factor(labels))], 0.4), pch = 19, axes = FALSE,

       frame.plot = TRUE, ann = FALSE, cex = 0.8)

  legend("bottomright", legend = round(aRI, 2), bty = "n",
